      ...
      <section className="w-full h-full flex flex-col md:flex-row gap-10 md:gap-6 md:justify-center">
        <div id="packageForm" className="w-full max-w-2xl shrink-0 grow-0 md:shrink md:grow md:max-h-full flex flex-col items-center mx-auto md:m-auto gap-3 md:gap-6 overflow-y-auto md:pr-2">
          ...
          <div className="w-full grid lg:grid-cols-2 gap-1 md:gap-y-2 md:gap-x-4">
            <Input value={packageData.name} onUpdateValue={(val) => onUpdatePackageData("name", val)} label="Name" placeholder="Enter package name"></Input>
            <Input value={packageData.description} onUpdateValue={(val) => onUpdatePackageData("description", val)} label="Description" placeholder="Enter package description"></Input>
            <Input value={packageData.price} onUpdateValue={(val) => onUpdatePackageData("price", val)} label="price" placeholder="Enter package price"></Input>
            <InputNumber value={packageData.width} onUpdateValue={(val) => onUpdatePackageData("width", val)} label="Width (m)" placeholder="Enter width" min="0"></InputNumber>
            <InputNumber value={packageData.height} onUpdateValue={(val) => onUpdatePackageData("height", val)} label="Height (m)" placeholder="Enter height" min="0"></InputNumber>
            <InputNumber value={packageData.length} onUpdateValue={(val) => onUpdatePackageData("length", val)} label="Length (m)" placeholder="Enter length" min="0"></InputNumber>
            <InputNumber value={packageData.weight} onUpdateValue={(val) => onUpdatePackageData("weight", val)} label="Weight (kg)" placeholder="Enter weight" min="0"></InputNumber>
          </div>
