...
export default function Modal({ open, width, height, hideCloseButton, className, children, onClose }) {
  ...
  return (
    <div className={`${display} w-full h-full fixed top-0 left-0 flex justify-center items-center bg-accent-100/20 px-2`}>
      <div style={{ width: `${width}px`, height: `${height}px` }} className={`${className} ${open ? "modalOpen" : "modalClose"} rounded-xl bg-ternary-100 flex flex-col`}>
        {!hideCloseButton ? (
